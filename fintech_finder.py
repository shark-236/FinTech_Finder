...
# Here we have added the "from crypto_wallet import *" statement into our "Imports" section, to keep all imports at the beginning of the code and to import all the functions we need for this file
...
# Here we create the variable named 'account'. It is set to an instrance of the generate_account() function from the crypto_wallet.py file
...
# Here we call the st.sidebar.write functionality. On another line we add the get_balance function, add w3, and parse in the account.address paremeter. 
...
# Here we create the wage variable and set it equal to the candidate's hourly rate (from the database) multiplied by the hours (inputted by the user)
wage = (candidate_database[person][3]) * hours


# @TODO
# Write the `wage` calculation to the Streamlit sidebar
# Here we use the st.sidebar.write functionality and pass in the 'wage' variable we previously created
...
if st.sidebar.button("Send Transaction"):
    ...
    # Here we call the send_transaction function and pass in the w3 parameter, as well as the 3 variables outlined above. 
    # Next, we save the function call as variable 'transaction hash' and add the .hex() functionality to convert the hex byte to a readable hash. 
    send_transaction(w3, account, candidate_address, wage)
    transaction_hash = send_transaction(w3, account, candidate_address, wage).hex()
...
# Here we add w3 to execute the function
get_people(w3)
